...
// declaramos variable const con un string
const saludo = "hola muchachos, hoy vamos a ver ciclos en javascript";

// declaramos una variable que va a mutar en cada iteracion del for, inicializamos con string vacio
let saludoParseado = "";

// generamos el for que recorrera todo tamaño del string
for (let index = 0; index < saludo.length; index++) {
  // dentro de cada iteracion verificamos si el caracter es un espacio
  if (saludo[index] === " ") {
    saludoParseado += "_"; // si es un espacio, de agrega guion bajo en vez de espacio al string parseado
  } else {
    saludoParseado += saludo[index]; // si no es espacio, se agrega la letra al string parseado
  }
}

// se imprime el resultado final del string parseado

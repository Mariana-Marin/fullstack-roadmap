...
//     if (bookCollection[i].chapters >= num) {
...
function getBooksByAmazonRating(rating) {
  ...
  for (let i = 0; i < bookCollection.length; i++) {
    for (let j = 0; j < bookCollection[i].ratings.length; j++) {
      if (bookCollection[i].ratings[j].source === "Amazon" && bookCollection[i].ratings[j].score >= rating) {
...
console.log(JSON.stringify(getBooksByAmazonRating(3), null, 2)); 

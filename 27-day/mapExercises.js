...
function getTitles(collection) {
  const arrTitles = collection.map((book) => book.title);

  return arrTitles;

  //return collection.map(book => book.title);
}
...
function getAuthorsUpperCase(collection) {
  return collection.map((book) => book.author.toUpperCase());
}
...
// Instruction: Use .map() to create a new array of objects. For each book,
// return a NEW object that only contains two properties: `title` (from the book's title) and
// `publicationYear` (from the book's publicationYear).
...
function getTitleAndYear(collection) {
  return collection.map((book) => {
    return { title: book.title, publicationYear: book.publicationYear };
  });
}
...
// Instruction: Use .map() to create an array of strings. For each book, return a string that follows this specific format:
//  "[Title] was published in [Year]".
...
function getBookInfoStrings(collection) {
  return collection.map((book) => {
    return `${book.title} was published in ${book.publicationYear}`;
  });
}

//console.log(getBookInfoStrings(bookCollection));

// 5. Exercise: Get Publisher Names
// Function Name: getPublisherNames
// Instruction: Use .map() to create an array containing just the name of the publisher
// for each book. Note that the `publisher` property is an object, so you need to access
// the `name` property inside it.
// Example Output: ["Chilton Books", "T. Egerton, Whitehall", ...]
// Hint: Access the nested property: `book.publisher.name`.

function getPublisherNames(collection) {
  return collection.map((book) => {
    return book.publisher.name;
  });
}

//console.log(getPublisherNames(bookCollection));

// 6. Exercise: Genre Counts
// Function Name: getGenreCounts
// Instruction: Use .map() to create an array of numbers. For each book,
// return the number of genres listed in its `genre` array.
...
function getGenreCounts(collection) {
  return collection.map((book) => {
    return book.genre.length;
  });
}

//console.log(getGenreCounts(bookCollection));

// 7. Exercise: First Keyword
// Function Name: getFirstKeyword
// Instruction: Use .map() to create an array containing only the FIRST keyword
// from each book's `keywords` array.
// Example Output: ["desert planet", "regency england", ...]
// Hint: Access the first element of the array using index 0: `book.keywords[0]`.

function getFirstKeyword(collection) {
  return collection.map((book) => {
    return book.keywords[0];
  });
}

console.log(getFirstKeyword(bookCollection));
...
function getSimplifiedBooks(collection) {
  return collection.map((book) => {
    return {
      title: book.title,
      author: book.author,
      isRecent: book.publicationYear > 2000,
    };
  });
}

console.log(getSimplifiedBooks(bookCollection));
...
// The `ratings` property is an array of objects. You need to calculate the average of the `score`
// properties from that array.
// Example Output: [4.525, 4.385, ...]
// Hint: Calculate the sum of scores (e.g., `book.ratings[0].score + book.ratings[1].score`)
// and divide by the number of ratings (`book.ratings.length`).

function getAverageRatings(collection) {
  return collection.map((book) => {
    return (
      (book.ratings[0].score + book.ratings[1].score) / book.ratings.length
    );
  });
}

console.log(getAverageRatings(bookCollection));
...
function reformatForFrontend(collection) {
  return collection.map((book) => {
    return { label: `${book.title} - ${book.author}`, value: book.isbn };
  });
}

console.log(reformatForFrontend(bookCollection));

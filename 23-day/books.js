...
the function should be named "getBooksByAvailability", the function should receive only 
one boolean parameter that is the availability that we want to get 
(true to get all available books and false to get the unavailable books)
...
function getBooksByAvailability(isAvailable) {
    const result = [];

    for (let i = 0; i < bookCollection.length; i++) {
        if (bookCollection[i].isAvailable === isAvailable) {
            result.push(bookCollection[i]);
        }
    }
...
function getBooksForYearsRange(initialYear, finalYear) {
    const result = [];
    for (let i = 0; i < bookCollection.length; i++) {
        if (bookCollection[i].publicationYear >= initialYear && bookCollection[i].publicationYear <= finalYear) {
            result.push(bookCollection[i]);
        }
    }
...
console.log(getBooksForYearsRange(2010, 2020));
...
// 1. Exercise: Total Pages
// Function Name: calculateTotalPages
// Instruction: Use .reduce() to calculate the total number of pages in the entire collection.
// Example Output: 6653 (or similar total)
// Hint: `acc + book.pages`

// 2. Exercise: Count Available Books
// Function Name: countAvailable
// Instruction: Use .reduce() to count how many books are currently available.
// Example Output: 18
// Hint: `book.isAvailable ? acc + 1 : acc`

// 3. Exercise: Concatenate Titles
// Function Name: concatenateTitles
// Instruction: Use .reduce() to create a single string containing all book titles, separated by commas.
// Example Output: "Dune, Pride and Prejudice, 1984, ..."
// Hint: `acc + (acc ? ", " : "") + book.title`

// 4. Exercise: Count by Genre
// Function Name: countByGenre
// Instruction: Use .reduce() to create an object where keys are genres and values are the number of books in that genre.
// Note: Since books have multiple genres, you'll need to iterate through `book.genre` inside the reduce callback.
// Example Output: { "Science Fiction": 5, "Fantasy": 3, ... }
// Hint: Initialize `acc` as `{}`. Loop through `book.genre` and update counts.

// 5. Exercise: Find Max Pages
// Function Name: findMaxPagesBook
// Instruction: Use .reduce() to find the book object with the highest number of pages.
// Example Output: { title: "The Name of the Wind", pages: 662, ... }
// Hint: `book.pages > acc.pages ? book : acc` (Initialize with the first book or a dummy object)

// 6. Exercise: Flatten Keywords
// Function Name: getAllKeywords
// Instruction: Use .reduce() to create a single array containing all keywords from all books (no duplicates if possible, but simple concatenation is fine for start).
// Example Output: ["desert planet", "ecology", "politics", ...]
// Hint: `[...acc, ...book.keywords]`

// 7. Exercise: Average Publication Year
// Function Name: averagePublicationYear
// Instruction: Use .reduce() to sum the publication years, then divide by the array length at the end.
// Example Output: 1964.5 (approx)
// Hint: `acc + book.publicationYear`. Return `total / length` after the reduce or inside a wrapper function.

// 8. Exercise: Group by Author
// Function Name: groupByAuthor
// Instruction: Use .reduce() to create an object where keys are author names and values are arrays of their book objects.
// Example Output: { "Frank Herbert": [{ title: "Dune", ... }], ... }
// Hint: Check if `acc[book.author]` exists. If not, create `[]`. Push `book`.

// 9. Exercise: Total Chapters
// Function Name: countTotalChapters
// Instruction: Use .reduce() to calculate the sum of all chapters in the collection.
// Example Output: 500+
// Hint: `acc + book.chapters`

// 10. Exercise: Highest Rated Book
// Function Name: findHighestRated
// Instruction: Use .reduce() to find the book with the highest "Goodreads" score.
// Example Output: { title: "The Name of the Wind", ... }
// Hint: Compare `book.ratings.find(r => r.source === "Goodreads").score` with `acc`'s score.

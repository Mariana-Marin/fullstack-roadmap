...
function createAndPrintNewObject(bookCollection) {
  let objectGenres = {};

  for (let i = 0; i < bookCollection.length; i++) {
    for (let j = 0; j < bookCollection[i].genre.length; j++) {
      let genre = bookCollection[i].genre[j];
      let pages = bookCollection[i].pages;
      if (objectGenres[genre]) {
        objectGenres[genre] += pages;
      } else {
        objectGenres[genre] = pages;
      }
    }
...
console.table(createAndPrintNewObject(bookCollection));
